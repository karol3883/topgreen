...
const galleryItems: GalleryItem[] = [
  {
    ...
    description: "Realizacja z mocną, spokojną geometrią frontu i wykończeniem dopasowanym do bryły domu.",
  },
  {
  ...
  },
  {
    ...
    description: "Przykład lżejszego systemu ogrodzeniowego utrzymanego w nowoczesnym języku wizualnym.",
  },
];
              ...
              {filteredItems.map((item) => (
                <article key={`${item.title}-${item.city}`} className="gallery-card reveal">
                  <button type="button" onClick={() => setSelectedItem(item)}>
                    <Image src={item.image} alt={item.alt} width={900} height={700} className="gallery-card-image" />
